...
if shlLeak:
    ...
    message = "At %s the shelter leak detector was activated.\n" % (tNow,)
    
    if not os.path.exists(os.path.join(STATE_DIR, 'inLeakDetected')):
    ...
    try:
        fh = open(os.path.join(STATE_DIR, 'inLeakDetected'), 'w')
        fh.write('%s\n' % tNow)
        fh.close()
    except Exception as e:
        print("Set water lock file: %s" % str(e))
        
elif os.path.exists(os.path.join(STATE_DIR, 'inLeakDetected')):
    # Check the age of the holding file to see if we have entered the "all-clear"
    age = time.time() - os.path.getmtime(os.path.join(STATE_DIR, 'inLeakDetected'))
    
    if age >= LEAK_CLEAR_TIME*60:
        tNow = shlTime.strftime("%B %d, %Y %H:%M:%S %Z")
        
        subject = '%s - Shelter Leak - Cleared' % (SITE.upper(),)
        message = "At %s the shelter leak detector was cleared.\n" % (tNow,)
        sendEmail(subject, message)
        
        try:
            os.unlink(os.path.join(STATE_DIR, 'inLeakDetected'))
        except Exception as e:
            print("Remove water lock file: %s" % str(e))
else:
    pass

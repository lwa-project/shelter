...
while 1:
	...
	if elapsed2 > 5:
                if initialised == True:
			startView = time.time()
			# Read the PDU info for each rack installed, every 10 sec
			for j in range(len(ri)):
				if (ri[j] == '1'):
	                        	# ================== Read the Power Distribution Units' Current and store value to MIB for PDU j+1
                                	...
                                	if verbose == 1:
                                        	print "Current from Rack", j+1, "is", PWRcurrent/10, "Amps."
                                	fp = open(MIB_FILE,'w')
                                	response = 'A' + string.rjust(str(me[0]),7)
                                	for i in range(len(mi)):
                                        	...
                                        	fp.writelines(mind[i]+' '+mi[i]+' '+me[i]+'\n')
                                	fp.close()
                                	# ================== Read the Power Distribution Units' port status and store value to MIB for PDU j+1
                                        for i in range(len(mi)):
                                            if mi[i]=='PORTS-AVAILABLE-R'+str(j+1):	#find how many port are active in rack j+1
                                                    numPorts = int(me[i])
                                        for k in range(numPorts):	#read each port status and overwrite it's MIB entry
                                            ...
                                            if PortStatus == 1:
                                                    ...
                                                    status = 'UNK'
                                            #print 'Rack '+str(j+1)+', Port '+str(k+1)+' is '+str(status)+'.'
                                            fp = open(MIB_FILE,'w')	
                                            for i in range(len(mi)):
                                                    ...
                                                    fp.writelines(mind[i]+' '+mi[i]+' '+me[i]+'\n')
                                            fp.close
			print "Elapased time checking PDU port status", time.time() - startView
             	start2 = time.time()
                                         ...
                                         for j in range(i,len(mi)):
                                             if mi[j]==port_index:
                                                ...
                                                if (rack == '1'):
                                                        ...
                                                        cmdgen.CommandGenerator().setCmd(pSecurity, pPDUNetwork[0], (eOutletCTL[int(port)-1], pdata))
                                                        if verbose == 1:
                                                            print 'Rack 1, Port '+str(port)+' has been changed to '+str(control)+'.'
                                                if (rack == '2'):
                                                        # Rack 2
                                                        errorIndication, errorStatus, errorIndex, varBinds = \
                                                        cmdgen.CommandGenerator().setCmd(p2Security, pPDUNetwork[1], (e2OutletCTL[int(port)-1], pdata))
                                                        if verbose == 1:
                                                            print 'Rack 2, Port '+str(port)+' has been changed to '+str(control)+'.'
                                                if (rack == '3'):
                                                        # Rack 3
                                                        errorIndication, errorStatus, errorIndex, varBinds = \
                                                        cmdgen.CommandGenerator().setCmd(p3Security, pPDUNetwork[2], (e3OutletCTL[int(port)-1], pdata))
                                                        if verbose == 1:
                                                            print 'Rack 3, Port '+str(port)+' has been changed to '+str(control)+'.'
                                                if (rack == '4'):
                                                        # Rack 4
                                                        errorIndication, errorStatus, errorIndex, varBinds = \
                                                        cmdgen.CommandGenerator().setCmd(p4Security, pPDUNetwork[3], (e4OutletCTL[int(port)-1], pdata))
                                                        if verbose == 1:
                                                            print 'Rack 4, Port '+str(port)+' has been changed to '+str(control)+'.'
                                                if (rack == '5'):
                                                        # Rack 5
                                                        errorIndication, errorStatus, errorIndex, varBinds = \
                                                        cmdgen.CommandGenerator().setCmd(p5Security, pPDUNetwork[4], (e5OutletCTL[int(port)-1], pdata))
                                                        if verbose == 1:
                                                            print 'Rack 5, Port '+str(port)+' has been changed to '+str(control)+'.'
                                                if (rack == '6'):
                                                        # Rack 6
                                                        errorIndication, errorStatus, errorIndex, varBinds = \
                                                        cmdgen.CommandGenerator().setCmd(p6Security, pPDUNetwork[5], (e6OutletCTL[int(port)-1], pdata))
                                                        if verbose == 1:

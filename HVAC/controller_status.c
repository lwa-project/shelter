...
int main(int argc, char* argv[]) {
	...
	value[0] /= found;
	value[1] /= found;
	if( value[0] > 2.5 && value[1] > 2.5 ) {
		printf("controller: on on\n");
	} else if( value[0] > 2.5 && value[1] <= 2.5 ) {
		printf("controller: on off\n");
	} else if( value[0] <= 2.5 && value[1] > 2.5 ) {
		printf("controller: off on\n");
	} else {
		printf("controller: off off\n");
	}
	sub_close(fh);
